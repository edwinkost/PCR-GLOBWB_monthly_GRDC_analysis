...
#~  layer(data = mergedTable, mapping = aes(x = date, y = simulation  ), geom = "line", colour = "blue", size = 0.35) + # model results
 layer(data = mergedTable, mapping = aes(x = date, y = observation), geom = "line" ) + # measurement
 layer(data = mergedTable, mapping = aes(x = date, y = simulation ), geom = "line" ) + # model results
#
